...
public class Tests : PageTest
{
    ...
    public async Task UserCanLoginAndFollowAnotherUser()
    {
        ...
        await Page.Locator("li").Filter(new() { HasText = "Jacqualine Gilcoine Follow Starbuck now is what we hear the worst. — 08/01/2023 " }).GetByRole(AriaRole.Button).First.ClickAsync();
    ...
    public async Task UserCannotFollowIfNotLoggedIn()
    {
        ...
        await Page.Locator("li").Filter(new() { HasText = "Jacqualine Gilcoine Follow Starbuck now is what we hear the worst. — 08/01/2023 " }).GetByRole(AriaRole.Button).First.ClickAsync();
        await Expect(Page).ToHaveURLAsync("https://bdsagroup2chirprazor.azurewebsites.net/Identity/Account/Register");
